(ns klj.reader-test
  ...
  (:use [klj.reader :only [read read-string]]
...
(deftest read-record
  (is (= (foo.) (read-string "#klj.reader_test.foo[]")))
  (is (= (foo.) (read-string "#klj.reader_test.foo []"))) ;; not valid in clojure
  (is (= (foo.) (read-string "#klj.reader_test.foo{}")))
  (is (= (assoc (foo.) :foo 'bar) (read-string "#klj.reader_test.foo{:foo bar}")))

  (is (= (map->bar {}) (read-string "#klj.reader_test.bar{}")))
  (is (= (bar. 1 nil) (read-string "#klj.reader_test.bar{:baz 1}")))
  (is (= (bar. 1 nil) (read-string "#klj.reader_test.bar[1 nil]")))
  (is (= (bar. 1 2) (read-string "#klj.reader_test.bar[1 2]"))))
...
(deftest preserve-read-cond
  ...
  ;; But currently, _any_ tag starting with \f gets treated as false.
  (is (= 1 (binding [*data-readers* {'foo (constantly 1)}]
             (read-string {:read-cond :preserve} "#foo []"))))
  ;; DMK: changing `foo` to `woo` lets test pass
  (is (= 1 (binding [*data-readers* {'woo (constantly 1)}]
             (read-string {:read-cond :preserve} "#woo []"))))
...
(deftest read-namespaced-map
  (binding [*ns* (the-ns 'klj.reader-test)]
...
(deftest read-symbol
  ...
  (testing "Not array class symbols, but symbols nevertheless"
    (doseq [suffix ["" "0" "11" "1a"]
      ...
      (is (= ((juxt namespace name) (read-string sym))
             ["String" suffix])))))
